...
class genQR:
    ...
    def __init__(self,root):
        ...
        #Studnet Name
        student_name_label = Label(left_frame,text="Roll.no:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        #reset button
        reset_btn=Button(btn_frame,command=self.genQR,text="Reset",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update stdattendance set id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='maty',host='localhost',database='face_recognizer',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from stdattendance where id=%s"
                    val=(self.var_id.get(),)
    ...
    def genQR(self):
           ...
           self.var_id.get()=="" or self.var_name.get()==""
           ...
           img.save(f"./qrImage/{id}.png")
                    ...
                    # self.var_id.get()==""
                    # n=self.var_id.get()
                    # with open("./att_reco/123.csv","r+",newline="\n") as f:
                    #     now=datetime.now()
